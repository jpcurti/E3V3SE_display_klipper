...
class PrinterData:
    ...
    def __init__(self, API_Key, Klipper_Socket, URL="127.0.0.1"):
        ...
        subscribe = {
            ...
            "params": {"objects": {"toolhead": ["position"]}, "response_template": {}},
        }
                        ...
                        if "z_offset" in status["configfile"]["config"]["bltouch"]:
                            if status["configfile"]["config"]["bltouch"]["z_offset"]:
                                self.BABY_Z_VAR = float(
                                    status["configfile"]["config"]["bltouch"][
                                        "z_offset"
                                    ]
                                )
    ...
    def init_Webservices(self):
        ...
        self.SHORT_BUILD_VERSION = self.getREST("/machine/update/status?refresh=false")[
            "result"
        ]["version_info"]["klipper"]["version"]
